...
- **Break screen**: point `pacer.breakComponent` at a global component; it mounts fullscreen in the break overlay while you keep navigating the deck
...
- The configured **break screen** (`pacer.breakComponent`, here `BreakDemo`) mounts fullscreen in the break overlay
...
- The break screen is a live component (no iframe), so you can **navigate the deck freely** behind it — advance slides, jump around; the overlay stays put
...
To verify: open `http://localhost:3030/` in one window and `http://localhost:3030/presenter` in another. Raise the break from the presenter window; the break screen + corner timer show in both. With no `breakComponent` set, you get a plain countdown overlay instead.
